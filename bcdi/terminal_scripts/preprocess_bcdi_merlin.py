...
if sys.argv[2].startswith("["):
    ...
    print(f"{root_folder}{sample_name}{scans[0]}/data/*mu*{scans[0]}*")
    # template_imagefile = 'NoMirror_ascan_mu_%05d_R.nxs'
    filename = glob.glob(f"{root_folder}{sample_name}{scans[0]}/data/*mu*{scans[0]}*")[0]
    template_imagefile = filename.split("/data/")[1].split("%05d"%scans[0])[0] +"%05d_R.nxs"

elif sys.argv[2]=="all":
    subdirnames = [x[1] for x in os.walk(f"{folder}/")][0]
    scans = [int(s.replace(sample_name, "")) for s in sorted(subdirnames) if s.startswith(sample_name)]
    print(scans)
    ...
    print(f"{root_folder}{sample_name}{scans}/data/*mu*{scans}*")
    # template_imagefile = 'NoMirror_ascan_mu_%05d_R.nxs'
    filename = glob.glob(f"{root_folder}{sample_name}{scans}/data/*mu*{scans}*")[0]
    template_imagefile = filename.split("/data/")[1].split("%05d"%scans)[0] +"%05d_R.nxs"
...
roi_detector = None
# roi_detector = [200, 515, 0, 515] # for merlin when we do not use center fft but skip
