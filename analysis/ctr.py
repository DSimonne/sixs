...
class Map(object):
    ...
    def __init__(self, folder, scan_indices):
        ...
        # Out of plane maps
        self.scan_list = [f for f in all_files if any(["-" + n + ".hdf5" in f for n in self.scan_indices])]
        ...
        self.qualitative_colors_2 = {"RT" : "#810f7c", "300" : "#66c2a5", "500" : "#fc8d62", "600" : "#8da0cb"}
    ...
    def prep_data(self, constant, variable, interpol_L_step = 0.01, CTR_width_H = 0.02, CTR_width_K = 0.02, 
        ...
        self.CTR_range_H = [self.HK_peak[0] - self.CTR_width_H, self.HK_peak[0] + self.CTR_width_H]
        self.CTR_range_K = [self.HK_peak[1] - self.CTR_width_H, self.HK_peak[1] + self.CTR_width_K]
        ...
        if center_background == HK_peak:
            ...
            self.background_range_H = [self.HK_peak[0] - self.background_width_H, self.HK_peak[0] + self.background_width_H]
            self.background_range_K = [self.HK_peak[1] - self.background_width_K, self.HK_peak[1] + self.background_width_K]

        else:
            self.background_width_H = background_width_H
            self.background_width_K = background_width_K

            self.background_range_H = [self.center_background[0] - self.background_width_H, self.center_background[0] + self.background_width_H]
            self.background_range_K = [self.center_background[1] - self.background_width_K, self.center_background[1] + self.background_width_K]
        ...
        for i, scan in enumerate(self.scan_list):
            ...
            with tb.open_file(self.folder + scan, "r") as f:
                ...
                end_H_roi = ut3.find_nearest(scan_h_axe, self.CTR_range_H[1])[0]

                st_K_roi = ut3.find_nearest(scan_k_axe, self.CTR_range_K[0])[0]
                end_K_roi = ut3.find_nearest(scan_k_axe, self.CTR_range_K[1])[0]
                ...
                tck = interpolate.splrep(scan_l_axe, roi_2D.sum(axis = (0, 1)), s = 0)
                span_data[i, 0, :] = interpolate.splev(self.l_axe, tck)
          
                if center_background == HK_peak:
                    # Background intensity, define roi indices
                    st_H_background = ut3.find_nearest(scan_h_axe, self.background_range_H[0])[0]
                    end_H_background = ut3.find_nearest(scan_h_axe, self.background_range_H[1])[0]

                    st_K_background = ut3.find_nearest(scan_k_axe, self.background_range_K[0])[0]
                    end_K_background = ut3.find_nearest(scan_k_axe, self.background_range_K[1])[0]

                    background_H = raw_data[st_H_background:end_H_background, st_K_roi:end_K_roi, :]
                    background_K = raw_data[st_H_roi:end_H_roi, st_K_background:end_K_background, :]

                    print(st_H_background, end_H_background)
                    print(st_K_background, end_K_background)

                    # Interpolate
                    tck_H = interpolate.splrep(scan_l_axe, background_H.sum(axis = (0, 1)), s = 0)
                    tck_K = interpolate.splrep(scan_l_axe, background_K.sum(axis = (0, 1)), s = 0)

                    span_data[i, 1, :] =  interpolate.splev(self.l_axe, tck_H) + interpolate.splev(self.l_axe, tck_K) - 2 * span_data[i, 1, :] 
                    ...
                    background_2D = raw_data[st_H_background:end_H_background, st_K_background:end_K_background, :]

                    # Interpolate
                    tck_2D = interpolate.splrep(scan_l_axe, background_2D.sum(axis = (0, 1)), s = 0)

                    span_data[i, 1, :] =  interpolate.splev(self.l_axe, tck_2D)


                self.names.append(scan.split(".hdf5")[0].split("_")[-1].replace("-", ":"))

        self.names = [x for _, x in sorted(zip([int(f.split(":")[-1]) for f in self.names], self.names))]
        np.save(self.folder + self.constant + self.variable + ".npy", span_data)
    ...
    def prep_data_fitaid(self, constant, variable, data_type = "nisf", CTR_width_H = 0.02, CTR_width_K = 0.02, 
        ...
        for i, scan in enumerate(self.scan_list):
            self.names.append(scan.split(".txt")[0].split("_")[-1].replace("-", ":"))
    ...
    def plot_CTR(self, scan_gas_dict, scan_temp_dict, figsize = (18, 6), fill_first = 0, fill_last = -1, zoom = [None, None, None, None], title = "CTR", save_as = "CTR.png"):
        ...
        if self.variable == "temp":
            self.labels = {scan_nb : f"{scan_temp_dict[scan_nb]} °C" for scan_nb in self.names}
            self.colors = {scan_nb : self.qualitative_colors_2[scan_temp_dict[scan_nb]] for scan_nb in self.names}

                    
        elif self.variable == "gas":
            self.labels = {scan_nb : f"{scan_gas_dict[scan_nb]}" for scan_nb in self.names}
            self.colors = {scan_nb : self.fivethirtyeight_colors[scan_gas_dict[scan_nb]] for scan_nb in self.names}
        ...
        # Otherwise as np array on disk
            ...
            span_data = np.load(self.folder + self.constant + self.variable + ".npy")
        ...
        plt.legend(bbox_to_anchor=(1,1), loc="upper left", fontsize = self.fontsize)
