...
class hdf5load(object):
    ...
    def __init__(self, directory, hdf5file):
        ...
        with tables.open_file(fullpath) as f:
            ...
            if Qindex == True:
                self.qaxe = np.linspace(self.Q[1],self.Q[2],1+self.Q[5]-self.Q[4])
                self.indaxe = np.linspace(self.index[1],self.index[2],1+self.index[5]-self.index[4])

            if hkl == True:
                print("Data shape:",self.data.shape)
                xaxe = np.arange(self.H[1],self.H[2],1+self.H[5]-self.H[4])
                self.haxe = np.linspace(self.H[1], self.H[2], 1 + int(self.H[5] - self.H[4])) #xaxe
                yaxe = np.arange(self.K[1],self.K[2],1+self.K[5]-self.K[4])
                self.kaxe = np.linspace(self.K[1], self.K[2], 1 + int(self.K[5] - self.K[4])) #yaxe
                zaxe = np.arange(self.L[1],self.L[2],1+self.L[5]-self.L[4])
                self.laxe = np.round(np.linspace(self.L[1], self.L[2], 1 + int(self.L[5] - self.L[4])), 3) #yaxe
                # self.laxe = np.round(np.linspace(self.L[1], self.L[2], int((self.L[2] - self.L[1])/self.L[3])), 3) #yaxe

            if QxQy == True:
                xaxe = np.linspace(self.X[1],self.X[2],1 + int(self.X[5]-self.X[4]))
                self.Qxaxe = xaxe
                yaxe = np.linspace(self.Y[1],self.Y[2],1 + int(self.Y[5]-self.Y[4]))
                self.Qyaxe = yaxe
                zaxe = np.linspace(self.Z[1],self.Z[2],1 + int(self.Z[5]-self.Z[4]))
                self.Qzaxe = zaxe
            ...
            print("hkl,QxQy,QparQper,Qphi,Qindex", hkl,QxQy,QparQper,Qphi,Qindex)
    ...
    def prjaxe(self, axe):
        ...
        if axe == 'Qx':
            datanan = np.swapaxes(self.dataRaw, 1, 2) ### to be check good for projection along Qx
            axenum = 0

        if axe == 'Qy':
            datanan = np.swapaxes(self.dataRaw, 0, 2) ### to be check good for projection along Qy
            axenum = 1

        if axe == 'Qz':
            datanan = np.swapaxes(self.dataRaw, 0, 1) ### to be check good for projection along Qz
            axenum = 2
    ...
    def prjaxe_range(self, axe, axerange):
        ...
        if axe == 'Qz':
            axenum = 2
            swap_data = np.swapaxes(self.dataRaw, 0, 1) ### to be check good for projection along Qz
        ...
        if axe == 'Qy':
            axenum = 1
            swap_data = np.swapaxes(self.dataRaw, 0, 2) ### to be check good for projection along Qy
        ...
        if axe == 'Qx':
            axenum = 0
            swap_data = np.swapaxes(self.dataRaw, 1, 2) ### to be check good for projection along Qx
    ...
    def hdf2tiff(self, axe, plot = 'YES', save = 'NO', axerange = None):
        ...
        if plot == 'YES':
            ...
            plt.tight_layout()
        if save =='YES':
            tiff = TIFF.open(self.directory + self.fn[0:-5] + '_' + axe + '.tiff', mode='w')
