...
def smoothSavgol(vect):
    ''' it applyes the kind of magic filter svagol to the y vector
...
def fitgauss1d_lim(x, y, aa=[10, 0.1, 100], ww=[0.2, 0.01, 0.6], cc=[0.8, 0.55, 1.1],
  ...
  # result = lmfit.minimize(gaus2min, params, args=(gamCritHole, yHole))
    ...
    if graph != 'NO':
        ...
        print(p)
        yfit = (p[0]*(1/p[1]*np.sqrt(2*np.pi)) *
...
def fitgauss1d2_lim(x, y, aa1=[10, 0.1, 100], ww1=[0.2, 0.01, 0.6], cc1=[0.8, 0.55, 1.1],
  ...
  # result = lmfit.minimize(gaus2min, params, args=(gamCritHole, yHole))
    ...
    if graph != 'NO':
        ...
        print(p1)
        yfit = (p1[0]*(1/p1[1]*np.sqrt(2*np.pi))*np.exp(-1/2 * ((x-p1[2])/p1[1])**2)) + p1[4] * \
            x + p1[3] + (p2[0]*(1/p2[1]*np.sqrt(2*np.pi)) *
                         np.exp(-1/2 * ((x-p2[2])/p2[1])**2))
        g1 = (p1[0]*(1/p1[1]*np.sqrt(2*np.pi)) *
              np.exp(-1/2 * ((x-p1[2])/p1[1])**2)) + p1[4]*x + p1[3]
        g2 = +p1[4]*x + p1[3] + (p2[0]*(1/p2[1]*np.sqrt(2*np.pi))
                                 * np.exp(-1/2 * ((x-p2[2])/p2[1])**2))
...
def fitPeaks(x, y, colorData='.g', plot='NO', print_it='NO'):
    ...
    # Variable Position,  Upper Limit, Lower Limit
    pars['d1_center'].set(397.6, min=397.3, max=397.9)
    ...
    # out is the object containing the results of the tit
    ...
    if plt != 'NO':   # just a quick plot way to visualise the fit results + data
        ...
        plt.show()
    if print_it != 'NO':
        # print report only if the fit result is minimally decent
        print(out.fit_report(min_correl=0.25))
    return init, out
...
def binto(x, y, Nintervals):
    ...
    n, bins = np.histogram(
        x, bins=Nintervals, range=(np.nanmin(x), np.nanmax(x)))
    fy, bins = np.histogram(x, bins=Nintervals, range=(
        np.nanmin(x), np.nanmax(x)), weights=y)
    ny = fy / n
...
def resample(x, y, Nintervals):
    "resample the x-y data and return a linearly spaced vectors"
    xst = x[0]
    xnd = x[-1]
    newx = np.linspace(xst, xnd, Nintervals)
