...
#        self._SpecNaNs = Nxs2Spec  # Remove the NaNs if the spec file need to be generated
        ...
        try:
            self._alias_dict = pickle.load(
                open('/home/andrea/MyPy3/sixs_nxsread/alias_dict.txt', 'rb'))
            #
        except FileNotFoundError:
            try:
                self._alias_dict_path = inspect.getfile(sixs).split("__")[
                    0] + 'alias_dict.txt'
                self._alias_dict = pickle.load(
        ...
        if self.scantype == 'FLY':
            ...
            try:  # adding just in case eventual missing attenuation
                if not hasattr(self, 'attenuation'):
                    self.attenuation = np.zeros(dataL)
            ...
            try:
                if not hasattr(self, 'attenuation_old'):
                    self.attenuation_old = np.zeros(dataL)
       ...
       ###################### Reading SBS ####################################
        ...
        try:  # kept for "files before 18/11/2020   related to xpad70/140 transition"
                ...
                for el in dets:
                    ...
                    if el == 'xpad140':
                        ...
                        self._distance_xpad140 = f.SIXS._f_get_child(
                            'i14-c-c00-ex-config-xpads140').distance_xpad[:]
                        self.attlist.append('_distance_xpad140')
                        self._ifmask_xpad140 = f.SIXS._f_get_child(
                            'i14-c-c00-ex-config-xpads140').ifmask[:]
                        self.attlist.append('_ifmask_xpad140')
                        try:
                            #print('check mask l 360')
                            self._mask_xpad140 = f.SIXS._f_get_child(
                                'i14-c-c00-ex-config-xpads140').mask[:]
                            self.attlist.append('_mask_xpad140')
                        except:
                            if self.verbose != 'NO':
            ...
            if self.verbose != 'NO':
                ...
                # raise #Exception
        try:
            if 'xpad140' in self._list2D:
                self._integration_time = f.SIXS._f_get_child(
                    'i14-c-c00-dt-xpad.s140').exposure_time[0]/1000
                self.attlist.append('_integration_time')
                if self.verbose != 'NO':
                    print('OK Int Time')

            if 'xpad70' in self._list2D:
                self._integration_time = f.SIXS._f_get_child(
                    'i14-c-c00-dt-xpad.s70').exposure_time[0]/1000
                self.attlist.append('_integration_time')
                if self.verbose != 'NO':
                    print('OK Int Time')
        except:
                    ...
                    for el in self._list2D:
                        try:
                            ...
                            for pos, roi in enumerate(self.__getattribute__('_roi_limits_' + el), start=0):
                                ...
                                roiname = 'roi'+str(pos) + '_'+el+'_new'
                                self.calcROI(
                                    stack, roi, maskname, self._integration_time, roiname, coef='default', filt='default')
                        except:
                            # raise
                            if self.verbose != 'NO':
