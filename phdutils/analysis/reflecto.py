...
class DataSets(object):
	...
	def __init__(self, folder, scan_indices, data_format, var = "l"):
		...
		all_files = [f.split("/")[-1] for f in sorted(glob.glob(f"{folder}/*.hdf5"))]

		if self.data_type == "hkl" and self.data_format == "hdf5":
		    self.scan_list = [f for f in all_files if any(["hkl_" + n + ".hdf5" in f for n in self.scan_indices])]

		elif self.data_type == "qxqyqz" and self.data_format == "hdf5":
		    self.scan_list = [f for f in all_files if any(["qxqyqz_" + n + ".hdf5" in f for n in self.scan_indices])]

		elif self.data_type == "qparqper" and self.data_format == "hdf5":
		    self.scan_list = [f for f in all_files if any(["qparqper_" + n + ".hdf5" in f for n in self.scan_indices])]

		elif self.data_type == "ang" and self.data_format == "hdf5":
		    self.scan_list = [f for f in all_files if any(["ang_" + n + ".hdf5" in f for n in self.scan_indices])]

		# data type not important for nxs files
		elif self.data_format == "nxs":
			all_files = [f.split("/")[-1] for f in sorted(glob.glob(f"{folder}/*.nxs"))]
			self.scan_list = [f for f in all_files if any([n + ".nxs" in f for n in self.scan_indices])]
		...
		self.BP_colors = {"Pt" : "#e41a1c", "BN" : "#377eb8", "Al2O3" : "#006d2c", "300" : "#66c2a5", "500" : "#fc8d62", "600" : "#8da0cb", "RT" : "#df65b0"}
	...
	def prep_binoc_data(self, wavel, tt_cutoff = 20, interpol_step = 0.03, CTR_range_h = None, CTR_range_k = None, qpar_range = None, qx_range = None, qy_range = None, delta_range = None, omega_range = None):
		...
		self.binoc_x_axis = np.arange(x_axis_min, x_axis_max, self.interpol_step)
		...
		if self.data_type == "hkl":
			...
			for i, fname in enumerate(self.scan_list):
				...
				with tb.open_file(self.folder + fname, "r") as f:
				    ...
				    scan_h_axe = np.linspace(h[1], h[2], 1 + int(h[5] - h[4])) #xaxe
				    scan_k_axe = np.linspace(k[1], k[2], 1 + int(k[5] - k[4])) #yaxe
				    scan_l_axe = np.linspace(l[1], l[2], 1 + int(l[5] - l[4])) #zaxe
				    ...
				    intensity = raw_data[st_h_roi:end_h_roi+1, st_k_roi:end_k_roi+1, :]

				    # Interpolate over common l axis
				    tck = interpolate.splrep(scan_l_axe, intensity.sum(axis = (0, 1)), s = 0)
				    
				    self.intensities.append(interpolate.splev(self.binoc_x_axis, tck))
			...
			for i, fname in enumerate(self.scan_list):
				...
				with tb.open_file(self.folder + fname, "r") as f:
				    ...
				    scan_qx_axe = np.linspace(qx[1], qx[2], 1 + int(qx[5] - qx[4])) #xaxe
				    scan_qy_axe = np.linspace(qy[1], qy[2], 1 + int(qy[5] - qy[4])) #yaxe
				    scan_qz_axe = np.linspace(qz[1], qz[2], 1 + int(qz[5] - qz[4])) #zaxe
				    ...
				    intensity = raw_data[st_qx_roi:end_qx_roi+1, st_qy_roi:end_qy_roi+1, :]

				    # Interpolate over common l axis
				    tck = interpolate.splrep(scan_qz_axe, intensity.sum(axis = (0, 1)), s = 0)
			...
			for i, fname in enumerate(self.scan_list):
				...
				with tb.open_file(self.folder + fname, "r") as f:
				    ...
				    scan_qpar_axe = np.linspace(qpar[1],  qpar[2], 1 + int(qpar[5] - qpar[4])) #yaxe
				    scan_qper_axe = np.linspace(qper[1], qper[2], 1 + int(qper[5] - qper[4])) #zaxe
				    print("Range in qpar:", qpar[1], qpar[2])
				    print("Range in qper:", qper[1], qper[2])

				    # CTR intensity, define roi indices
				    st_qpar = ut3.find_nearest(scan_qpar_axe, qpar_range[0])[0]
				    end_qpar = ut3.find_nearest(scan_qpar_axe, qpar_range[1])[0]
			...
			for i, fname in enumerate(self.scan_list):
				...
				with tb.open_file(self.folder + fname, "r") as f:
				    ...
				    scan_delta_axe = np.linspace(delta[1], delta[2], 1 + int(delta[5] - delta[4])) #xaxe
				    scan_omega_axe = np.linspace(omega[1], omega[2], 1 + int(omega[5] - omega[4])) #yaxe
				    scan_gamma_axe = np.linspace(gamma[1], gamma[2], 1 + int(gamma[5] - gamma[4])) #zaxe
				    ...
				    # CTR intensity, define roi indices
				    st_delta_roi = ut3.find_nearest(scan_delta_axe, delta_range[0])[0]
				    end_delta_roi = ut3.find_nearest(scan_delta_axe, delta_range[1])[0]
				    print("Indexes in delta: ", st_delta_roi, end_delta_roi+1)

				    st_omega_roi = ut3.find_nearest(scan_omega_axe, omega_range[0])[0]
				    end_omega_roi = ut3.find_nearest(scan_omega_axe, omega_range[1])[0]
				    print("Indexes in omega: ", st_omega_roi, end_omega_roi+1)

				    intensity = raw_data[st_delta_roi:end_delta_roi+1, : , st_omega_roi:end_omega_roi+1].sum(axis = (0, 2))
	...
	def compute_miller(self, wl, tt_cutoff = 20):
		...
		self.Pt_PD = xu.simpack.PowderDiffraction(xu.materials.Pt, wl = wl, tt_cutoff = tt_cutoff).data

		self.Al2O3_PD = xu.simpack.PowderDiffraction(xu.materials.Al2O3, wl = wl, tt_cutoff = tt_cutoff).data

		self.BN = xu.materials.Crystal.fromCIF("/home/david/Documents/PhD_local/PhDScripts/Surface_Diffraction/BN_mp-984_conventional_standard.cif")
		self.BN_PD = xu.simpack.PowderDiffraction(self.BN, wl = wl, tt_cutoff = tt_cutoff).data

		del self.BN_PD[(0, 0, 1)]
		del self.BN_PD[(0, 0, 3)]

		self.theta_bragg_pos_BN = [(miller_indices, self.BN_PD[miller_indices]["ang"]) for miller_indices in self.BN_PD]
		self.theta_bragg_pos_Al2O3 = [(miller_indices, self.Al2O3_PD[miller_indices]["ang"]) for miller_indices in self.Al2O3_PD]
		self.theta_bragg_pos_Pt = [(miller_indices, self.Pt_PD[miller_indices]["ang"]) for miller_indices in self.Pt_PD]

		self.q_bragg_pos_BN = [(miller_indices, self.BN_PD[miller_indices]["qpos"]) for miller_indices in self.BN_PD]
		self.q_bragg_pos_Al2O3 = [(miller_indices, self.Al2O3_PD[miller_indices]["qpos"]) for miller_indices in self.Al2O3_PD]
		self.q_bragg_pos_Pt = [(miller_indices, self.Pt_PD[miller_indices]["qpos"]) for miller_indices in self.Pt_PD]
	...
	def plot_refl(self, title, save_as, figsize = (18, 9), labels = False, ncol = 2, scan_gas_dict = False, x_axis = "l", y_max = False, y_min = False, x_min = False, x_max = False, fill = False, fill_first = 0, fill_last = -1, miller = False, critical_angles = False, background = True, low_range =False, high_range = False):
		...
		if not low_range and not high_range:
			...
			for (i, y), x, scan_index in zip(enumerate(self.intensities), self.x_axis, self.scan_indices):
				if isinstance(background, str):
					...
					y_plot = np.where((y-new_bck < self.background_bottom), self.background_bottom, y-new_bck)
			...
			try:
				plt.axhline(y = self.y_og, color = 'black', linewidth = self.linewidth_hline, alpha = self.alpha_hline)
			except:
				pass

			# Generate a bolded vertical line at x = 0 to highlight origin
			plt.axvline(x = 0, color = 'black', linewidth = self.linewidth_vline, alpha = self.alpha_vline)
			...
			if miller:
				if self.data_format == "hdf5" and self.data_type == "qxqyqz":
					...
					Pt_pos = [(a, 2*b + 0.5) for (a, b) in self.theta_bragg_pos_Pt[:1]]
					BN_pos = [(a, 2*b + 0.5) for (a, b) in self.theta_bragg_pos_BN]
					Al2P3_pos = [(a, 2*b + 0.5) for (a, b) in self.theta_bragg_pos_Al2O3]
					plot_peaks = True
			...
			# Ticks
			plt.xticks(np.arange((self.x_min//self.x_tick_step)*self.x_tick_step, (self.x_max//self.x_tick_step)*self.x_tick_step, self.x_tick_step), fontsize = self.tick_fontsize)
			plt.yticks(np.arange((self.x_min//self.x_tick_step)*self.x_tick_step, (self.x_max//self.x_tick_step)*self.x_tick_step, self.x_tick_step), fontsize = self.tick_fontsize)
		...
		################################################# TWO FIGURES ###########################################""
			...
			fig, axes = plt.subplots(1, 2, sharey = True, figsize = figsize, dpi = 150)
			...
			# Generate a bolded vertical line at x = 0 to highlight origin
			axes[0].axvline(x = 0.01, color = 'gray', linewidth = self.linewidth_vline, alpha = self.alpha_vline)
			...
			if self.data_format == "hdf5" and self.var == "hkl":
				...
				fig.text(x = 0.5, y = -0.04, s = "Gamma (°)", fontsize = 30, ha='center')
	...
	def compare_roi(self, index_range = 1010, v_min_log = 0.01, v_max_log = 100, central_pixel_gamma = 25, central_pixel_delta = 285):
		...
		def plot2D(a, b, c, d, reflecto, index):
			...
			ax[0].axvline(central_pixel_delta, linestyle = "--", linewidth = 1, color = "black")
			ax[0].axhline(central_pixel_gamma, linestyle = "--", linewidth = 1, color = "black")
		...
		_list_widgets = interactive(plot2D,
		                    ...
		                    reflecto = widgets.Select(
		                        options = [rn4.DataSet(self.folder + f) for f in self.scan_list],
		                        description = 'Scan:',
                                style = {'description_width': 'initial'},
