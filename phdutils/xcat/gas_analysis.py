...
class XCAT():
    ...
    def __init__(self, log_file):
        ...
        self.pathtodata = self.log_file if self.log_file.endswith(
            ".txt") else self.log_file + ".txt"
        ...
        print(
            f"Travel time from cell to detector fixed to {self.gaz_travel_time} seconds for now ...")
        ...
        self.heater_poly_fit_coef = np.polyfit(
            self.heater_ramp["Current"], self.heater_ramp["Temperature"], 1)
    ...
    def separate_xcat_dataframes(self, entry_list=False):
        ...
        if entry_list:
            ...
            self.entry_list = ["no", "h2", "o2", "co",
                               "ar", "shunt", "reactor", "drain", "valve"]
    ...
    def load_rga(self, rga_file, skiprows=33):
        ...
        try:
            ...
            new_time_column = np.round(np.linspace(
                0, self.duration, self.duration + 1), 0)
            ...
            print(
                "To interpolate also the rga data, you need to run Valves.load_rga() first")
    ...
    def truncate_xcat_df(self):
        ...
        try:
            for entry in self.entry_list:
                ...
                index_start_time = (
                    temp_df[f"time_{entry}"] - self.time_stamp).abs().argsort()[0]
                ...
                if self.duration:
                    ...
                    temp_df = temp_df[temp_df[f"time_{entry}"]
                                      < self.duration + 1]

                    setattr(self, f"{entry}_df_truncated", temp_df)

                    print(
                        f"New truncated dataframe created starting from timestamp for {entry} and for {self.duration} seconds.")
    ...
    def interpolate_xcat_df(self):
        ...
        try:
            for entry in self.entry_list:
                ...
                # make sure that the time is integer
                interpolated_df[f"time_{entry}"] = interpolated_df[f"time_{entry}"].astype(
                    int)
    ...
    def plot_xcat_entry(self,
        ...
        try:
            ...
            for entry in plot_entry_list:
                ...
                axes[0].plot(
                    plot_df[f"time_{entry}"], plot_df[f"flow_{entry}"], label=f"flow_{entry}")

                axes[0].plot(
                    plot_df[f"time_{entry}"], plot_df[f"setpoint_{entry}"], "--", label=f"setpoint_{entry}")

                axes[1].plot(
                    plot_df[f"time_{entry}"], plot_df[f"valve_{entry}"], "--", label=f"valve_{entry}")
                ...
                try:
                    ...
                    for cursor_pos, cursor_lab in zip(cursor_positions, cursor_labels):
                        axes[0].axvline(
                            cursor_pos, linestyle="--", color=self.ammonia_conditions_colors[cursor_lab], linewidth=1.5, label=cursor_lab)
                        axes[1].axvline(
                            cursor_pos, linestyle="--", color=self.ammonia_conditions_colors[cursor_lab], linewidth=1.5, label=cursor_lab)
    ...
    def plot_xcat_valves(self,
        ...
        try:
            ...
            axes[0].plot(plot_df["time_valve"],
                         plot_df["valve_MRS"], label="valve_MRS")
            axes[1].plot(plot_df["time_valve"],
                         plot_df["valve_MIX"], label="valve_MIX")
    ...
    def plot_rga_norm_ptot(self,
        ...
        self.norm_df_ptot = pd.DataFrame(
            used_arr / val, columns=norm_df.columns)
