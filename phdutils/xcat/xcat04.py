...
class DataSet(object):
    ...
    def __init__(self, directory,filename): 
        ...
        self.data = np.genfromtxt(self.longname, dtype=float, skip_header=1, delimiter='')
    ...
    def _XCATtime(self, TimeVector, TimeRange=None):
        ...
        '''
        fullEpoch = TimeVector - 2082844800  #transform the xcat epoch (epoch labview time) in unix epoch
        if not TimeRange:
            ...
            #print 'Time Window'
            Dstart, Dstart_value = ut3.find_nearest(fullEpoch,TimeRange[0]) #min(matplotlib.mlab.find(tt > TimeRange[0])) #TimeRange[0]=start
            timeEND = TimeRange[0] + TimeRange[1]                 #TimeRange[1]=duration         
            Dend, Dend_value = ut3.find_nearest(fullEpoch, timeEND)      #max(matplotlib.mlab.find(tt<timeEND))
            if len(fullEpoch[Dstart:Dend])<1:
                print('######   No Epoch/Timerange Overlap     #######')
            return Dstart, Dend, fullEpoch[Dstart:Dend]                                # position within the vector
        pass
    ...
    def selectGas(self, TimeRange=None):
        ...
        H2st, H2nd, H2 =  self._XCATtime(self.data[:,4], TimeRange=TimeRange) #select the time range between Dstart et Dend
        ...
        O2st, O2nd, O2 =  self._XCATtime(self.data[:,8], TimeRange=TimeRange) #select the time range between Dstart et Dend
        ...
        COst, COnd, CO =  self._XCATtime(self.data[:,12], TimeRange=TimeRange) #select the time range between Dstart et Dend
        ...
        Arst, Arnd, Ar =  self._XCATtime(self.data[:,16], TimeRange=TimeRange) #select the time range between Dstart et Dend
        ...
        Shuntst, Shuntnd, Shunt =  self._XCATtime(self.data[:,20], TimeRange=TimeRange) #select the time range between Dstart et Dend
        ...
        Reactorst, Reactornd, Reactor =  self._XCATtime(self.data[:,24], TimeRange=TimeRange) #select the time range between Dstart et Dend
        Reactor=np.c_[Reactor,self.data[Reactorst:Reactornd,25]]                     #flow Range
        Reactor=np.c_[Reactor,self.data[Reactorst:Reactornd,26]]                    #setpoint  Range  
        Reactor=np.c_[Reactor,self.data[Reactorst:Reactornd,27]]                     #valve Range
        self.Reactor = Reactor
               
        Drainst, Drainnd, Drain =  self._XCATtime(self.data[:,28], TimeRange=TimeRange) #select the time range between Dstart et Dend
        ...
        Valvesst, Valvesnd, Valves =  self._XCATtime(self.data[:,32], TimeRange=TimeRange) #select the time range between Dstart et Dend
        Valves=np.c_[Valves,self.data[Valvesst:Valvesnd,33]]                     #MIX
        Valves=np.c_[Valves,self.data[Valvesst:Valvesnd,34]]                     # MRS
    ...
    def getSection(self, TimeRange, mixPos=3, mrsPos=10 ):
        ...
        mrsd = matplotlib.mlab.find(np.diff(mrs)!=0) # here we search only the points where there is a variation in the mrs state
        
        SectionStarts = []
        #print('inter is: ',inter)
        for el in mrsd:
            #print('element is: ',el)
            if (np.average(mrs[el+3:el+6]) == mrsPos) & (np.average(mix[el+3:el+6]) == mixPos): # take few data points after the valve movements since the valve movement is not immediate nor constant in shift
                SectionStarts.append(timePhase[el]) # This if do not exclude to have two values one after the others registered while the valve is moving
    ...
    def Gas7Plot(self, filenameXCAT, filenameRGA, masslist, channelList,TimeRange=None):   
        ...
        plt.figure(num=None, figsize=(11, 15), dpi=80, facecolor='w', edgecolor='k')
        # GasLog=self.selectGas(TimeRange=TimeRange)

        channelpos=list()
        channels=('NO','H2','O2','CO','Ar','Shunt','Reactor','Drain','Valves')
        ...
        for el, index_el in zip(channelList, channelpos):
            ...
            plt.plot(flow_data[:,0], flow_data[:,1], colors[index_el], label = channels[index_el])
        ...
        for el, index_el in zip(channelList, channelpos):
            flow_data = getattr(self, el)
            plt.plot(flow_data[:,0], flow_data[:,2], colors[index_el], label = channels[index_el])
        ...
        for el, index_el in zip(channelList, channelpos):
            flow_data = getattr(self, el)
            plt.plot(flow_data[:,0], flow_data[:,3], colors[index_el], label = channels[index_el])
...
# NO, H2, O2, CO, Ar, Valves, Shunt, Reactor, Drain    
    ...
    for el1 in timePoints:
        ...
        for el2 in GasLogPos[0:5]:
            ...
            TotFlux = TotFlux + GasLog[el2][Dstart,2] # I read the set value!!!!
        #print (TotFlux)
...
def gasSlopes(Xcatfile,RgaFile,masses,shift = 15,inter = 30,TimeRange=None,savefile='NO', corrF_Ar = None, corrF_Flux = None):
    ...
    BatchStarts = getSection(Xcatfile, mixPos=3, mrsPos=10, TimeRange=TimeRange)
    BatchStarts = BatchStarts - TimeRange[0]
    print (BatchStarts)
    ScaleFactors =  gasScalingFactor(Xcatfile ,BatchStarts, TimeRange=TimeRange, corrF_Ar = corrF_Ar, corrF_Flux = corrF_Flux)
    #print (ScaleFactors)
    batches = []
    #shift = 15 # delay time between batch start and flow stop
    for el in BatchStarts:
        el = el + TimeRange[0]
        Interval = [el+shift,inter] # define the interval to fit: [starting point, delta]
#        print(Interval)
        batch = RGAsrs.RgaSelect(RgaFile, masses,TimeRange=Interval) ### ==> computationally poor step, I have to read the file n times
#        print (np.shape(batch))
        batches.append(batch) # batches is a list of matrices, each matrix is a time 
...
#    return batches
    ...
    slopebatches = []
    for index1,el in enumerate(BatchStarts): # 1st loop goes over each batch phase (using the lenght of batchstarts)
        slopeMasses = []
        slopeError = []
...
#            print (batches[index1][1,index2])
            ...
            slope, intercept, r_value, p_value, std_err = stats.linregress(x,ynorm)
...
def gasSlopes3(Xcatfile, RgaFile, masses, Pres, shift = 15,inter = 30,TimeRange=None,savefile='NO'):#, corrF_Ar = None, corrF_Flux = None): 
    '''Load the masses e.g. [40,28,32,44] from RGA/XCAt files for the desired experiment and in the 
    selected time range it looks for the batch phases and fits the first 20 seconds of each batch phase 
    with a linear interpolation returning the slope for each gas-batch session.
    ...
    for el in BatchStarts:
        el = el + TimeRange[0]
        Interval = [el+shift,inter] # define the interval to fit: [starting point, delta]
        batch = RGAsrs.RgaNormPtotPlot(RgaFile, masses, Pres,TimeRange = Interval, plot = 'NO') # RGAsrs.RgaSelect(RgaFile, masses,TimeRange=Interval) ### ==> computationally poor step, I have to read the file n times
...
### start from mass 1 because because mass 0 is the carrier gas
        ...
        if index1 == 0: #
            ...
            if savefile!= 'NO':
                ...
                f.writelines('   '.join('%5g'%e for e in slopeMasses) +'   '+str(BatchStarts[index1])+ '\n')
                g.writelines('   '.join('%5g'%e for e in slopeError) +'   '+str(BatchStarts[index1])+ '\n')
        else:            
            #slopebatches = np.vstack([slopebatches,slopeMasses])
            if savefile!= 'NO':
                f.writelines('   '.join('%5g'%e for e in slopeMasses) +'   '+str(BatchStarts[index1])+ '\n')
...
def gasSlopesMapPar(Xcatfile,RgaFile,masses,shi,inte,TimeRange=None,savefile='NO',corrF_Ar = None, corrF_Flux = None):
    ...
    for shi in shifts:
        ...
        for interv in intervals:
            batchTime,slopes = gasSlopes(Xcatfile,RgaFile,masses,TimeRange=None,savefile='NO',shift = shi,inter = interv, corrF_Ar = corrF_Ar, corrF_Flux = corrF_Flux)
...
def gasFlow(Xcatfile,RgaFile,masses,shift = -20,inter = 15,TimeRange=None,savefile='NO', corrF_Ar = None, corrF_Flux = None):
    ...
    for el in BatchStarts:
        ...
        Interval = [el+shift,inter] # define the interval to fit: [starting point, delta]
#        print(Interval)
        flow = RGAsrs.RgaSelect(RgaFile, masses,TimeRange=Interval) ### ==> computationally poor step, I have to read the file n times
#        print (np.shape(batch))
        flows.append(flow) # flows is a list of matrices, each matrix is a time 
                              # column and RGA data referring to a desired mass at the beginning of the (batch) phase
#    here we go in for the slope for each gas.        
#    return batches
...
def gasFlow2(Xcatfile,RgaFile,masses,Pres,shift = -20,inter = 15,TimeRange=None,savefile='NO', corrF_Ar = None, corrF_Flux = None):
    ...
    for el in BatchStarts:
        el = el + TimeRange[0]
        Interval = [el+shift,inter] # define the interval to fit: [starting point, delta]
        flow = RGAsrs.RgaNormPtotPlot(RgaFile, masses, Pres,TimeRange = Interval, plot = None) ### ==> computationally poor step, I have to read the file n times
        flows.append(flow) # flows is a list of matrices, each matrix is a time 
                              # column and RGA data referring to a desired mass at the beginning of the (batch) phase
#    here we go in for the slope for each gas.        
#    return batches
...
## ==> (Preactor - Pin)/(Pin / flowIn) ==> flow variation    
    data[:,0] = ((data[:,0]-p0CO)/(p0CO/0.005))/Vol# * .005)/Vol #( Preactor - Pinitial / Pinitial )*entrance Flow ==> consumed flow/Volume for normalisation
