...
def RgaLoad(filename):
    ...
    HeaderST='Channel,  Mass(amu),' # here the strings to search in the header of the file as reference points
    HeaderEND='Time(s) '
    sttime='Start time, '
    channel=[]
    masses=[]
    with open(filename) as myFile:
        ...
        for num, line in enumerate(myFile, 1):
            if sttime in line:
                ...
                #print( timeline[12:])
                starttime = time.mktime(datetime.datetime.strptime(timeline[12:], "%b %d, %Y  %I:%M:%S  %p").timetuple())
            ...
            if num > hst:
                ...
                if np.shape(LineFields)[0]>2: # if first field of "line shape" > 2 AKA exclude empty lines
...
def RgaPlot(filename, masses, TimeRange=None, style='none'):
    ...
    for el in masses:
        ...
        if style!='XKCD':
            ...
            plt.plot(timeEXP[Dstart:Dend],vect[Dstart:Dend],label='Mass '+str(el))
        
        elif style=='XKCD':
            with plt.xkcd():
                figXKCD=plt.figure()
                figXKCD.plot(timeEXP[Dstart:Dend],vect[Dstart:Dend],label='Mass '+str(el))
...
def RgaNormPtotPlot(filename, masses, Pres ,TimeRange = None, plot = None):
    ...
    if plot != 'NO':
        for index, el in enumerate(masses):
            plt.plot(vectAllnorm[:,0],vectAllnorm[:,index+1],linewidth=2, label='Mass '+str(el))
        pl.semilogy()
