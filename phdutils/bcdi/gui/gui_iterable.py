...
class Dataset():
    ...
    def to_ghr(self):
        ...
        try:
            ...
            with tb.open_file(f"{self.name}.ghr", "w", title = f"{self.name}, X-ray spectroscopy dataset, processed via thorondor.") as f:
                
                # Create nexus entry
                f.create_group("/","NXentry", "Entry following ghr rules. Handle with pytables.")
